...
public class ConversionTools {
	...
	/**
	 * int类型转换成byte类型（高位）保持最低字节中各个位不变，3个高字节全部用0填充
	 * 
	 * @param intValue
	 * @return
	 */
	public static byte[] intToBytesH(int intValue) {
		byte[] returnByte = new byte[4];
	    for (int i = 0; (i < 4) && (i < 4); i++) {
	    	returnByte[i] = (byte) (intValue >> 8 * i & 0xFF);
	    }
		return returnByte;
	}
	/**
	 ...
	 * @return
	 */
	public static int bytesToIntH(byte[] byteValue) {
		int returnValue = 0;
	    for (int i = 0; i < byteValue.length; i++) {
	    	returnValue += (byteValue[i] & 0xFF) << (8 * i);
	    }
	    return returnValue;
	}
