...
public class InterfaceAction extends BaseAction {
	...
	public String execute() {
		try {
			...
			if (checkRequest()) {
				...
				if (data.length < 1) {
					this.thorwError("", ParameterCode.Result.RESULT_FAIL,
							ParameterCode.Error.SERVICE_INVALID, "无效的请求包体", "");
					return null;
				}
				String bodyString = ConversionTools
						.bytesToString(data, "utf-8");
				...
				if (methodCode.equals(MethodCode.ACCOUNT_REGISTER)
					...
					Account currentAccount = (Account) session
							.get(Configure.systemSessionAccount);
					if (currentAccount == null) {
						this.thorwError("", ParameterCode.Result.RESULT_FAIL,
								ParameterCode.Error.SERVICE_SESSION,
								"回话已经过期，请重新登录。", "");
						return null;
					}
					if (methodCode.equals(MethodCode.GET_INDEX_DOC_TYPES)) {
						returnString = queryService
								.getIndexDocTypes(bodyString);
					} else if (methodCode.equals(MethodCode.CREATE_SCENE)) {
						returnString = sceneService.createScene(bodyString,
								currentAccount);
						...
						returnString = sceneService.getSceneList(bodyString,
								currentAccount);
					} else if (methodCode.equals(MethodCode.HISTORY_SCENE)) {
						returnString = sceneService.getSceneHistory(bodyString,
								currentAccount);
					} else if (methodCode.equals(MethodCode.SWITCH_SCENE)) {
						returnString = sceneService.switchScene(bodyString,
								currentAccount, session);
					} else if (methodCode.equals(MethodCode.SWITCH_VERSION)) {
						returnString = sceneService.switchSceneVersion(
								bodyString, currentAccount);
					} else if (methodCode.equals(MethodCode.GET_SCENE_VERSION)) {
						returnString = sceneService.getSceneVersion(bodyString,
								currentAccount);
					} else if (methodCode.equals(MethodCode.CLOSE_VERSION)) {
						returnString = sceneService.closeSceneVersion(
								bodyString, currentAccount);
					} else if (methodCode.equals(MethodCode.UPDATE_SCENE)) {
						returnString = sceneService.updateScene(bodyString,
								currentAccount, session);
					} else if (methodCode.equals(MethodCode.CREATE_FRAGMENT)) {
						returnString = fragmentService.createFragment(
								bodyString, currentAccount);
					} else if (methodCode.equals(MethodCode.UPDATE_FRAGMENT)) {
						returnString = fragmentService.updateFragment(
								bodyString, currentAccount);
					} else if (methodCode.equals(MethodCode.GET_FRAGMENT)) {
						returnString = fragmentService.getFragment(bodyString,
								currentAccount);
					} else if (methodCode.equals(MethodCode.DELETE_FRAGMENT)) {
						returnString = fragmentService.deleteFragment(
								bodyString, currentAccount);
					} else if (methodCode.equals(MethodCode.DISABLE_FRAGMENT)) {
						returnString = fragmentService.disableFragment(
								bodyString, currentAccount);
					} else if (methodCode.equals(MethodCode.ENABLE_FRAGMENT)) {
						returnString = fragmentService.enableFragment(
								bodyString, currentAccount);
					} else if (methodCode
							.equals(MethodCode.DISABLE_SHARE_FRAGMENT)) {
						returnString = fragmentService.disableShareFragment(
								bodyString, currentAccount);
					} else if (methodCode
							.equals(MethodCode.ENABLE_SHARE_FRAGMENT)) {
						returnString = fragmentService.enableShareFragment(
								bodyString, currentAccount);
					} else if (methodCode.equals(MethodCode.LIST_FRAGMENT)) {
						returnString = fragmentService.getFragments(bodyString,
								currentAccount);
					} else if (methodCode
							.equals(MethodCode.LIST_SHARE_FRAGMENT)) {
						returnString = fragmentService.getShareFragments(
								bodyString, currentAccount);
					} else if (methodCode.equals(MethodCode.CREATE_VARIABLE)) {
						returnString = variableService.createVariable(
								bodyString, currentAccount);
					} else if (methodCode.equals(MethodCode.GET_VARIABLE)) {
						returnString = variableService.getVariable(bodyString,
								currentAccount);
					} else if (methodCode.equals(MethodCode.UPDATE_VARIABLE)) {
						returnString = variableService.updateVariable(
								bodyString, currentAccount);
					} else if (methodCode.equals(MethodCode.DELETE_VARIABLE)) {
						returnString = variableService.deleteVariable(
								bodyString, currentAccount);
					} else if (methodCode
							.equals(MethodCode.DISABLE_SHARE_VARIABLE)) {
						returnString = variableService.disableShareVariable(
								bodyString, currentAccount);
					} else if (methodCode
							.equals(MethodCode.ENABLE_SHARE_VARIABLE)) {
						returnString = variableService.enableShareVariable(
								bodyString, currentAccount);
					} else if (methodCode.equals(MethodCode.LIST_VARIABLE)) {
						returnString = variableService.getVariables(bodyString,
								currentAccount);
					} else if (methodCode
							.equals(MethodCode.LIST_SHARE_VARIABLE)) {
						returnString = variableService.getShareVariables(
								bodyString, currentAccount);
					} else {
						this.thorwError("", ParameterCode.Result.RESULT_FAIL,
				...
				if (StringUtil.isNullOrEmpty(returnString)) {
					this.thorwError("", ParameterCode.Result.RESULT_FAIL,
							ParameterCode.Error.SERVICE_RESOLVE, "服务器业务处理错误",
							"");
					return null;
				}
				response.getOutputStream()
						.write(returnString.getBytes("utf-8"));
				return null;
			} else {
				this.thorwError("", ParameterCode.Result.RESULT_FAIL,
						ParameterCode.Error.SERVICE_REFUSE, "请求被服务器拒绝", "");
				return null;
			}
		} catch (Exception e) {
			e.printStackTrace();
			...
			this.thorwError("", ParameterCode.Result.RESULT_FAIL,
					ParameterCode.Error.SERVICE_UNKNOWN, "服务器未知错误", "");
			return null;
		}
	}
	...
	private void thorwError(String optCode, String statusCode,
